...
docker buildx build \
  --file "$DOCKER_BUILD_PATH"/"$DOCKERFILE" \
  --output type=image,name="$ECR_REGISTRY/$ECR_REPO_NAME:latest,$GITHUB_SHA",oci-mediatypes=true,compression=zstd,compression-level=3,force-compression=true,push=true "$DOCKER_BUILD_PATH"
